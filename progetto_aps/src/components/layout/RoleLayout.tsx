...
export default function RoleLayout({ children, title }: Props) {
    ...
    const roleForBadge: "PAT" | "HOSP" | "LAB" = (user?.role ?? "PAT");

    return (
        <div className="min-h-screen bg-slate-50 text-slate-900">
            <header className="sticky top-0 z-20 h-14 bg-white/70 backdrop-blur border-b">
...
function RoleBadge({ role }: { role: "PAT" | "HOSP" | "LAB" }) {
    const styles =
        role === "PAT" ? "bg-emerald-100 text-emerald-700"
            : role === "HOSP" ? "bg-indigo-100 text-indigo-700"
                : "bg-amber-100 text-amber-700";
    const label = role === "PAT" ? "Paziente" : role === "HOSP" ? "Ospedale/Medico" : "Laboratorio";
