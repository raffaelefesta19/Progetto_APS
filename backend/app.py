...
def lab_emit():
    ...
    return jsonify({"ok": True, "envelope": env})
...
def hosp_open():
    db = load_db()
    b = get_json_body()
    ok, msg = require_fields(b, ("reportId", "hospitalId", "labId"))
    if not ok:
        return jsonify({"ok": False, "error": msg}), 400
    ...
    # Enforcement revoca applicativa del paziente
...
def dev_seed():
    """
    Crea utenti demo (pat1/lab1/hosp1) + 3 referti DEMO-R-0001..3.
    ...
    def emit_share_revoke(report_id: str, exam_type: str, result_short: str, note: str,
                          content: str, share_with_hosp: bool, do_revoke: bool):
        db_local = load_db()
    ...
    if emit_share_revoke(
        ...
        share_with_hosp=True, do_revoke=False
    ):
    ...
    if emit_share_revoke(
        ...
        share_with_hosp=False, do_revoke=True
    ):
        seeded.append("DEMO-R-0003")

    return jsonify({
        "ok": True,
        "users": {"pat": pat_uid, "lab": lab_uid, "hosp": hosp_uid},
        "seeded": seeded
    })
